...
int comp_float(const void* a, const void* b) { return *(float*)a - *(float*)b; }
...
void ground_truth(int n_pts, int d, int n_query, char* path_data, char* query_data, char* path_gt) {
  ...
  for (int i = 0; i < n_pts; i++) data[i] = new float[d];
  ...
  for (int i = 0; i < n_query; i++) query[i] = new float[d];
...
void index(int n_pts, int d, int n_sample, const float* data_ptr, const std::string& index_data) {
  ...
  for (int i = 0; i < n_pts; i++)
    for (int j = 0; j < d; j++) data[i][j] = 0;

  for (int i = 0; i < n_pts; ++i) {
    std::copy(data_ptr + i * d_org, data_ptr + (i + 1) * d_org, data[i]);
  }
  ...
  for (int i = 0; i < L; i++) vec_1[i] = new double[d / 2];

  double** vec_2 = new double*[L];
  for (int i = 0; i < L; i++) vec_2[i] = new double[d / 2];

  float** train = new float*[n_sample];
  for (int i = 0; i < n_sample; i++) train[i] = new float[d];
  ...
  for (int i = 0; i < n_sample; i++) train1[i] = new float[d / 2];

  float** train2 = new float*[n_sample];
  for (int i = 0; i < n_sample; i++) train2[i] = new float[d / 2];
  ...
  for (int i = 0; i < n_pts; i++) {
    ...
    for (int j = 0; j < d; j++) array1[table_id][n_temp[table_id]].val += residual_vec[i][j] * residual_vec[i][j];
  ...
  for (int i = 0; i < size; i++) vec2[i] = new double*[L];
  for (int i = 0; i < size; i++) {
    for (int j = 0; j < L; j++) {
  ...
  for (int i = 0; i < m; ++i) output.write((char*)(proj_array[i]), sizeof(float) * d);
  ...
  for (int i = 0; i < n_pts; i++) {
    for (int j = 0; j < m_level; j++) {
      ...
      for (int l = 0; l < m_num; l++) {
        float ssum = 0;
        for (int ll = 0; ll < d; ll++) ssum += residual_vec[i][ll] * proj_array[j * m_num + l][ll];

        if (ssum >= 0) {
          code_num += 1;
        }

        if (l < m_num - 1) code_num = code_num << 1;
      }
  ...
  for (int i = 0; i < n_pts; i++) proj_val[i] = new float[m];
    ...
    for (int n = 0; n < n_pts; n++) {
      for (int i = 0; i < M2; i++) {
        for (int j = 0; j < L; j++) {
          sum = 0;
          for (int l = 0; l < d2; l++) {
            sum += (residual_vec[n][i * d2 + l] - vec2[k * M2 + i][j][l]) *
                   (residual_vec[n][i * d2 + l] - vec2[k * M2 + i][j][l]);
          }
...
//   fmt::println("DataPath: {}", data_path);
//   ifstream input(data_path, ios::binary);
//   if (!input.is_open()) {
//     fmt::println("Open file {} error", data_path);
//     exit(-1);
//   }
//   if (data_type == "ds") {
//     fmt::println("DataType: {}", data_type);
//     for (int i = 0; i < n_pts; i++) {
//       float t;
//       input.read((char*)(data[i]), sizeof(float) * d_org);
//       input.read((char*)&t, sizeof(float));  // read additional dimension
...
//   ofstream output(index_path, ios::binary);
...
//   ifstream input(path_data, ios::binary);
//   if (!input.is_open()) {
//     fmt::println("Open file {} error", path_data);
//     exit(-1);
//   }
//   for (int i = 0; i < n_pts; i++) {
//     float t;
...
void search(int n_pts, int n_query, int d, int topk, float delta, int l0, int flag_, const float* query_ptr,
            const float* data_ptr, std::vector<std::vector<unsigned>>& gt, const char* path_index) {
  int real_topk = topk;
  topk = 100;
  ...
  float** proj_array = new float*[m];
  for (int i = 0; i < m; ++i) proj_array[i] = new float[d];
  ...
  int q_dim = d_org + 1;
  for (int i = 0; i < n_query; i++) {
    std::copy(query_ptr + i * q_dim, query_ptr + (i + 1) * q_dim, query[i]);
    float norm_q = calc_norm(query[i], d);
    for (int j = 0; j < d; ++j) {
      query[i][j] = query[i][j] / norm_q;
    }
    u[i] = -1 * query_ptr[i * q_dim + d_org] / norm_q;
  }

  ifstream input_index(path_index, ios::binary);

  float** vec1 = new float*[L];
  for (int i = 0; i < L; i++) vec1[i] = new float[d / 2];

  float** vec2 = new float*[L];
  for (int i = 0; i < L; i++) vec2[i] = new float[d / 2];
  ...
  int size_per_element_ = sizeof(int) + 2 * sizeof(float) + sizeof(unsigned long) * m_level +
  ...
  for (int i = 0; i < m; i++) input_index.read((char*)(proj_array[i]), sizeof(float) * d);
  ...
  for (int i = 0; i < level; i++) {
    ...
    for (int k = 0; k < n_cand1; k++) {
      for (int j = 0; j < count[k]; j++) {
        ...
        char* cur_loc = index_[k] + j * size_per_element_ + sizeof(int) + 2 * sizeof(float) +
                        sizeof(unsigned long) * m_level + i * (M2 + sizeof(float) + sizeof(unsigned long) * m_level);
    ...
    for (int j = 0; j < n_pts; j++) {
      for (int jj = 0; jj < m_level; jj++) input_index.read((char*)&(code_num[j][jj]), sizeof(unsigned long));
    }
  ...
  for (int i = 0; i < size; i++)
    for (int j = 0; j < L; j++) vec_pq[i][j] = new float[d2];
  ...
  for (int i = 0; i < level; i++)
    for (int j = 0; j < M2; j++) table2[i][j] = new float[L];
  ...
  for (int i = 0; i < n_query; i++) {
    ...
    for (int j = 0; j < m_level; j++) {
      ...
      for (int jj = 0; jj < m_num; jj++) {
        ...
        if (jj < m_num - 1) query_proj[j] = query_proj[j] << 1;
      }
    }
  ...
  // }
  for (int i = 0; i < n_query; ++i) {
    std::vector<bool> flag(real_topk, false);
    for (int j = 0; j < real_topk; ++j) {
      for (int k = 0; k < real_topk; ++k) {
        if (!flag[k] && gt[i][j] == search_result[i][k]) {
          correct++;
          flag[k] = true;
          break;
        }
...
//   int** search_result = new int*[n_query];
//   for (int i = 0; i < n_query; i++) search_result[i] = new int[topk];
//   float** val_result = new float*[n_query];
//   for (int i = 0; i < n_query; ++i) val_result[i] = new float[topk];
...
//   std::string base_data = fmt::format("{}/{}.{}", kFrontPath, kDataset, kDataType);
//   fmt::println("base_data: {}", base_data);
//   ifstream input_data(base_data, ios::binary);
//   if (!input_data.is_open()) {
//     fmt::println("Open file {} error", base_data);
//     exit(-1);
//   }
//   if (kDataType == "ds") {
//     fmt::println("DataType: {}", kDataType);
//     for (int i = 0; i < n_pts; i++) {
//       float t;
//       input_data.read((char*)(data[i]), 4 * d_org);
//       input_data.read((char*)&t, sizeof(float));
...
//   // ifstream input_query(query_data, ios::binary);
//   std::string query_data = fmt::format("{}/{}.{}", kFrontPath, kDataset, kQueryType);
//   fmt::println("query path: {}", query_data);
//   ifstream input_query(query_data, ios::binary);
//   if (!input_query.is_open()) {
//     fmt::println("Open file {} error", query_data);
//     exit(-1);
//   }
//   for (int i = 0; i < n_query; i++) {
//     input_query.read((char*)(query[i]), sizeof(float) * d_org);
...
//   std::string path_index = fmt::format("{}/{}.mqh", kFrontPath, kDataset);
//   fmt::println("Load index from {}", path_index);
//   ifstream input_index(path_index, ios::binary);
//   if (!input_index.is_open()) {
//     fmt::println("Open file {} error", path_index);
//     exit(-1);
//   }
...
//     for (int j = 0; j < topk; j++) {
//       search_result[i][j] = retset[j].id;
//       val_result[i][j] = retset[j].distance;
//     }
//   }
//   timer.end();

//   fmt::println("{} Search Time: {}", kDataset, timer.getElapsedTime() / n_query);
//   // float time_us_per_query = stopw.getElapsedTimeMicro() / n_query;

//   constexpr unsigned kGtSize = 1000;
//   unsigned* gt_id = nullptr;
//   std::string kGtPath = fmt::format("{}/{}@{}.igt", kFrontPath, kDataset, kGtSize);
//   utils::read_data<unsigned>(kGtPath, n_query, kGtSize, gt_id);
//   float recall = 0;
//   for (int i = 0; i < n_query; ++i) {
//     unsigned* gt = gt_id + i * kGtSize;
//     for (int j = 0; j < real_topk; ++j) {
//       // fmt::print("({}, {}, {}), ", gt[j], search_result[i][j], val_result[i][j]);
//       for (int k = 0; k < real_topk; ++k) {
//         if (gt[j] == search_result[i][k]) {
//           recall += 1;
//           break;
//         }
//       }
...
//     // fmt::print("\n");
//   }
//   fmt::println("{} Recall: {}", kDataset, recall / (n_query * real_topk));
// }

// void search(int n_pts, int n_query, int d, int topk, float delta, int l0, int flag_, char* query_data, char*
// base_data,
//             char* path_gt, char* path_index) {
//   int real_topk = topk;
//   topk = 100;
...
//   for (int i = 0; i < n_query; ++i) {
//     std::ignore = fscanf(fp, "%d", &tmp1);
//     for (int j = 0; j < maxk; ++j) {
//       float tmp3;
//       std::ignore = fscanf(fp, ",%d,%f", massQA + maxk * i + j, &tmp3);
//     }
//     std::ignore = fscanf(fp, "\n");
//   }
//   fclose(fp);
